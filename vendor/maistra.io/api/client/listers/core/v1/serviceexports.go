...
type ServiceExportsLister interface {
	...
	List(selector labels.Selector) (ret []*v1.ServiceExports, err error)
...
// List lists all ServiceExports in the indexer.
func (s *serviceExportsLister) List(selector labels.Selector) (ret []*v1.ServiceExports, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ServiceExports))
	})
	return ret, err
}
...
type ServiceExportsNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.ServiceExports, error)
...
// List lists all ServiceExports in the indexer for a given namespace.
func (s serviceExportsNamespaceLister) List(selector labels.Selector) (ret []*v1.ServiceExports, err error) {
...
// Get retrieves the ServiceExports from the indexer for a given namespace and name.
func (s serviceExportsNamespaceLister) Get(name string) (*v1.ServiceExports, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("serviceexports"), name)
	}
	return obj.(*v1.ServiceExports), nil
}
