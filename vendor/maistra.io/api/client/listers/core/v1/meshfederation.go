...
type MeshFederationLister interface {
	...
	List(selector labels.Selector) (ret []*v1.MeshFederation, err error)
...
// List lists all MeshFederations in the indexer.
func (s *meshFederationLister) List(selector labels.Selector) (ret []*v1.MeshFederation, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.MeshFederation))
	})
	return ret, err
}
...
type MeshFederationNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.MeshFederation, error)
...
// List lists all MeshFederations in the indexer for a given namespace.
func (s meshFederationNamespaceLister) List(selector labels.Selector) (ret []*v1.MeshFederation, err error) {
...
// Get retrieves the MeshFederation from the indexer for a given namespace and name.
func (s meshFederationNamespaceLister) Get(name string) (*v1.MeshFederation, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("meshfederation"), name)
	}
	return obj.(*v1.MeshFederation), nil
}
