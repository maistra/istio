...
func createCommand(args []string) *cobra.Command {
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			mrc, err := memberroll.NewControllerFromConfigFile(
				"",
				namespace,
				"default",
				resyncPeriod,
	...
	cmd.PersistentFlags().StringVar(&tokenPath, "tokenPath", "/var/run/secrets/kubernetes.io/serviceaccount/token", "File containing to the ServiceAccount token to be used for communication with the K8s API server")
	cmd.PersistentFlags().StringVar(&serveDirectory, "serveDirectory", "/srv", "Directory form where WASM modules are served")
	cmd.PersistentFlags().StringVar(&registryURL, "registryURL", "image-registry.openshift-image-registry.svc:5000", "Registry from which to pull images by default")
