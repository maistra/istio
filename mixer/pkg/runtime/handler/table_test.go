...
func TestNew_EmptyConfig(t *testing.T) {
	...
	table := NewTable(Empty(), s, nil, nil, []string{metav1.NamespaceAll})
...
func TestNew_Reuse(t *testing.T) {
	...
	table2 := NewTable(table, s, nil, nil, []string{metav1.NamespaceAll})
...
func TestCleanup_WorkerNotClosed(t *testing.T) {
	...
	for idx, tt := range tests {
		t.Run(fmt.Sprintf("[%d]", idx), func(t *testing.T) {
			...
			gp.AddWorkers(5)
			oldTable := NewTable(Empty(), s, gp, nil, []string{metav1.NamespaceAll})
			...
			newTable := NewTable(oldTable, s, nil, nil, []string{metav1.NamespaceAll})
...
func TestCleanup_WithStartupError(t *testing.T) {
	...
	table2 := NewTable(table, config.Empty(), nil, nil, []string{metav1.NamespaceAll})
