...
func TestIntoResourceFile(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// Verifies that istio-cni is appended to existing k8s.v1.cni.cncf.io/networks string value
		...
		{
			// Verifies that istio-cni is appended to existing k8s.v1.cni.cncf.io/networks JSON value
			in:   "hello-existing-cncf-networks-json.yaml",
			want: "hello-existing-cncf-networks-json.injected.yaml",
			setFlags: []string{
				"components.cni.enabled=true",
				"values.istio_cni.chained=false",
			},
