...
type Controller struct {
	*kubecontroller.Controller
	...
	cs           *clientsetservicemeshv1alpha1.Clientset
	env          *model.Environment
...
func NewController(opt Options) *Controller {
	...
	cs := clientsetservicemeshv1alpha1.NewForConfigOrDie(opt.Client.RESTConfig())

	var informer cache.SharedIndexInformer
	if opt.Client.GetMemberRoll() != nil {
		...
		opt.Client.GetMemberRoll().Register(namespaceSet, "federation-controller")
	} else {
		informer = informersservicemeshv1alpha1.NewMeshFederationInformer(
			cs, opt.Namespace, opt.ResyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc})
	}
...
func (c *Controller) reconcile(resourceName string) error {
	...
	instance, err := c.cs.MaistraV1alpha1().MeshFederations(namespace).Get(
		ctx, name, metav1.GetOptions{TypeMeta: metav1.TypeMeta{Kind: "MeshFederation", APIVersion: v1alpha1.SchemeGroupVersion.String()}})
	if err != nil {
...
func (c *Controller) update(ctx context.Context, instance *v1alpha1.MeshFederation) error {
	...
	egressGatewayService := fmt.Sprintf("%s.%s.svc.cluster.local", instance.Spec.Gateways.Egress.Name, instance.Namespace)
	...
	if registry != nil {
		...
		if registry.Provider() != serviceregistry.Federation {
			return fmt.Errorf("cannot create Federation registry for %s, registry exists and belongs to another provider (%s)", instance.Name, registry.Provider())
		}
...
func (c *Controller) delete(_ context.Context, instance *v1alpha1.MeshFederation) error {
	registry := c.getRegistry(instance.Name)
	if registry != nil {
		// make sure it's one of ours
		if registry.Provider() != serviceregistry.Federation {
			return fmt.Errorf("cannot delete Federation registry for %s, registry belongs to another provider (%s)", instance.Name, registry.Provider())
		}
