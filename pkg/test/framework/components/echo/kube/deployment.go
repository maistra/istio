...
func newDeployment(ctx resource.Context, cfg echo.Config) (*deployment, error) {
	...
	if err = ctx.Config(cfg.Cluster).ApplyYAML(cfg.Namespace.Name(), deploymentYAML); err != nil {
...
func (d *deployment) WorkloadReady(w *workload) {
	...
	if err := d.ctx.Config(d.cfg.Cluster.Primary()).ApplyYAML(d.cfg.Namespace.Name(), wle); err != nil {
