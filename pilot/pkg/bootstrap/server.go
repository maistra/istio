...
func NewServer(args PilotArgs) (*Server, error) {
	...
	if args.Config.ControllerOptions.MemberRollName != "" {
		if args.Namespace != "" {
			...
			args.Config.ControllerOptions.WatchedNamespaces = model.IstioSystemNamespace
		}
	}
...
func (s *Server) initMemberRollController(args *PilotArgs) error {
	if hasKubeRegistry(args) && args.Config.FileDir == "" && args.Config.ControllerOptions.MemberRollName != "" {
		mrc, err := controller.NewMemberRollControllerFromConfigFile(s.getKubeCfgFile(args), args.Namespace,
...
func (s *Server) makeKubeConfigController(args *PilotArgs) (model.ConfigStoreCache, error) {
	...
	return crd.NewController(configClient, s.mrc, args.Config.ControllerOptions), nil
}
...
func (s *Server) createK8sServiceControllers(serviceControllers *aggregate.Controller, args *PilotArgs) (err error) {
	...
	kubectl := kube.NewController(s.kubeClient, s.mrc, args.Config.ControllerOptions)
