...
//nolint:lll
func (c *Controller) AdditionalPodSubscriptions(_ *model.Proxy, _ sets.Set[types.NamespacedName], _ sets.Set[types.NamespacedName]) sets.Set[types.NamespacedName] {
	// Currently, Ambient is not supported
	return sets.New[types.NamespacedName]()
}

func (c *Controller) Policies(_ sets.Set[model.ConfigKey]) []*workloadapi.Authorization {
	// Currently, Ambient is not supported
	return []*workloadapi.Authorization{}
}

func (c *Controller) Waypoint(_ model.WaypointScope) sets.Set[netip.Addr] {
	// Currently, Ambient is not supported
	return sets.New[netip.Addr]()
}
